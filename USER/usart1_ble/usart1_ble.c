...
u8 receive_str[UART1_REC_NUM];     //接收缓存数组,最大USART_REC_LEN个字节 
u8 send_str[UART1_TRS_NUM]; 
...
{ 
	u16 k= 0 ; 
   	do 
   	{ 
   		uart1SendChar(*(str + k)); k++; 
   	}while (k < strlen); //循环发送,直到发送完毕  
} 

__align(8) u8  send_str[UART1_TRS_NUM];
...
{  
	...
	while (len--)
	{
		...
		USART_SendData(USART1,send_str[cnt++]);
	}                        
}
...
void USART1_IRQHandler(void)  
{
	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)  //接收中断 
	{
		...
		else if(rec_data=='E')		                         //如果E，表示是命令信息传送的结束位
		{
			...
			{
				uart1SendChars("correct_OKcorrect_OKcorrect_OK",30);
			}
			...
			else if(strcmp("send_voi",(char *)receive_str)==0)
			{
			    ...
			    USART_SendData(USART1,0x1a);
			    */
				voice = (float)Get_Adc(ADC_Channel_5) / 4096.0f * 430.0 / 5.0;
		...
		else if((uart_byte_count>0)&&(uart_byte_count<=UART1_REC_NUM))
		{
