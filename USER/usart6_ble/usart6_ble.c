...
__align(8) u8  send_str[UART6_TRS_NUM];
...
{  
	...
	while (len--)
	{
		...
		USART_SendData(USART6,send_str[cnt++]);
	}                        
}
...
void USART6_IRQHandler(void)  
{
	...
	if(USART_GetITStatus(USART6, USART_IT_RXNE) != RESET)  //接收中断 
	{
		...
		else if(rec_data=='E')		                         //如果E，表示是命令信息传送的结束位
		{
			...
			{
				uart6SendChars("correct_OKcorrect_OKcorrect_OK",30);
			}
			else if(strcmp("send_acc",(char *)receive_str)==0)
			{
				...
				USART_SendData(USART6,0x1a);
				*/
				sprintf(send_str, "%.3f%s",a[0],"m/s2");
				len = strlen((const char*)send_str);
				uart6SendChars(send_str,len);
		...
		else if((uart_byte_count>0)&&(uart_byte_count<=UART6_REC_NUM))
		{
